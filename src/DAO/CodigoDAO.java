...
public class CodigoDAO
{
  ...
  public void setUsoMaximoCodigos(int QtdDispMaxTotal, int QtdDispMaxKis, int QtdDiasMaxTotal1A, int QtdDiasMaxTotal2A, int QtdDiasMaxTotal3A, int QtdDiasMaxKIS1A, int QtdDiasMaxSmall1A, int QtdDiasMaxSmall2A)
  {
    try
    {
      PreparedStatement stmt = con.prepareStatement("update qtd_uso_cods set TOTAL1_disp = ?,KIS1_disp = ?,TOTAL1_dias = ?,TOTAL2_dias = ?,TOTAL3_dias = ?,KIS1_dias = ?,SMALL1_dias = ?,SMALL2_dias = ?");
  ...
  public List<Codigos> buscaTodosOsCodigos()
  {
    try
    {
      ...
      PreparedStatement stmt = con.prepareStatement("select * from codigos;");
      ResultSet rs = stmt.executeQuery();
      while (rs.next()) {
  ...
  public HashMap<Integer, Codigos> buscaTodosOsCodigosMap()
  {
    try
    {
      ...
      while (rs.next()) {
        ...
        mapCod.put(Integer.valueOf(cod.getId()), cod);
      }
      return mapCod;
    } catch (SQLException e) {
  ...
  public ArrayList<Codigos> buscaTodosOsCodigos(String tipo, String situacao, int anos)
  {
    try
    {
      ...
      PreparedStatement stmt = con.prepareStatement("select * from codigos where tipo = ?" + situacao + " and duracao = ? order by id and data_expiracao;");
  ...
  public ArrayList<Codigos> getCodigosUtilizaveis()
  {
    ...
    try {
      ...
      ArrayList<Codigos> listTodosCod2 = verificaDesbloqueioEQuantidadeUsada(listTodosCod1);
  ...
  public void adicionar_valor_venda_no_codigo(Vendas v, int id) {
    ...
    try {
      PreparedStatement stmt = con.prepareStatement("update codigos c set c.valor_total_vendas = c.valor_total_vendas + (? * ?) where c.id = ?");
      
      stmt.setDouble(1, valordispositivo);
      stmt.setDouble(2, qtddispvenda);
      stmt.setInt(3, id);
      PreparedStatement stmt2 = con.prepareStatement("update codigos c set c.valor_total_lucro = c.valor_total_lucro + (? * ?) where c.id = ?;");
  ...
  private ArrayList<Codigos> verificaDesbloqueioEQuantidadeUsada(ArrayList<Codigos> list1) {
    ...
    for (Codigos c : list1) {
      ...
      if ((c.getTipo().toLowerCase().equals("kis")) || (c.getTipo().toLowerCase().equals("android"))) {
        if ((map.containsKey(Integer.valueOf(c.getId()))) || (c.getQtd_usada() <= 100))
        {
          if ((!map.containsKey(Integer.valueOf(c.getId()))) || (((Integer)map.get(Integer.valueOf(c.getId()))).intValue() != 1) || (c.getQtd_usada() <= 140))
          {
            if ((!map.containsKey(Integer.valueOf(c.getId()))) || (((Integer)map.get(Integer.valueOf(c.getId()))).intValue() != 2))
            {

              list2.add(c); } }
        }
      } else if (c.getTipo().toLowerCase().equals("total")) {
        if ((!map.containsKey(Integer.valueOf(c.getId()))) || (!map.containsKey(Integer.valueOf(c.getId()))) || (c.getQtd_usada() <= 50))
        {
          if ((!map.containsKey(Integer.valueOf(c.getId()))) || (((Integer)map.get(Integer.valueOf(c.getId()))).intValue() != 1) || (c.getQtd_usada() <= 60))
          {
            if ((map.containsKey(Integer.valueOf(c.getId()))) && (((Integer)map.get(Integer.valueOf(c.getId()))).intValue() == 2)) {
              list2.add(c);
            } else
  ...
  public void updateQtdUsadaCodigo(int id)
  {
    try
    {
      ...
      ResultSet rs = stmt.executeQuery();
      while (rs.next()) {
        int qtd = rs.getInt("qtd_dispositivos");
  ...
  public ArrayList<Codigos> getCodigosProximosExpirar() { ArrayList list = new ArrayList();
    try {
