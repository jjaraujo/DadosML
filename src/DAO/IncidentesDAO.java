...
public class IncidentesDAO
{
  ...
  public ArrayList<Incidentes> getIncidentes() {
    try {
      ...
      PreparedStatement stmt = VariaveisDeControle.CON.prepareStatement("select i.id id, i.id_venda id_venda, i.id_codigo id_codigo,v.apelido_comprador apelido,c.codigo codigo,v.data data_compra, i.data_criacao data_criacao, mi.motivo_texto motivo_texto, i.situacao situacao, i.anotacoes anotacoes from incidentes i join motivo_incidentes mi on i.motivo = mi.id  join vendas v on i.id_venda = v.id join codigos c on i.id_codigo = c.id where i.situacao = 'ABERTO'");
