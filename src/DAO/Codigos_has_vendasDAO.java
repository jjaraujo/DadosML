...
public class Codigos_has_vendasDAO
{
  ...
  public codigos_has_vendas getUmaVenda(String id, int idCod) {
    try {
      ...
      PreparedStatement stmt = con.prepareStatement("select * from codigos_has_vendas where id_venda like '" + id + "' and id_codigo = " + idCod + ";");
  ...
  public ArrayList<codigos_has_vendas> getTodasAsVendas()
  {
    try {
      ...
      PreparedStatement stmt = con.prepareStatement("select * from codigos_has_vendas;");
      ResultSet rs = stmt.executeQuery();
      while (rs.next()) {
        codigos_has_vendas cod = new codigos_has_vendas();
        cod.setIdVenda(rs.getString("id_venda"));
  ...
  public ArrayList<codigos_has_vendas> getCodigoHasVendas(String idVenda)
  {
    ...
    try
    {
      PreparedStatement stmt = con.prepareStatement("select * from codigos_has_vendas where id_venda like ?;");
  ...
  public ArrayList<Codigos> getCodigosJaEnviadosParaOCliente(String apelido) {
    ...
    try
    {
      PreparedStatement stmt = con.prepareStatement("select cd.id id, cd.codigo codigo from codigos_has_vendas chv join vendas v on v.id = chv.id_venda join codigos cd on chv.id_codigo = cd.id where apelido_comprador like ?");
  ...
  public boolean insertCodigoEmVenda(codigos_has_vendas chv)
  {
    try
    {
      PreparedStatement stmt = con.prepareStatement("insert into codigos_has_vendas (id_venda,id_codigo,qtd_dispositivos,qtd_servidor) values (?,?,?,?) ;");
  ...
  public void updateCodigoHasVendas(codigos_has_vendas chv) {
    try {
      PreparedStatement stmt = con.prepareCall("update codigos_has_vendas set qtd_dispositivos = ?,qtd_servidor = ? where id_venda like ? and id_codigo = ?;");
  ...
  public void updateCodigoAntigos(codigos_has_vendas chv, String codigo) {
    try {
      PreparedStatement stmt = con.prepareCall("update codigos_has_vendas set codigos_antigos = concat(codigos_antigos,?) where id_venda like ?;");
