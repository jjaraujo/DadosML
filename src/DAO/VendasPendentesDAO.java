...
public class VendasPendentesDAO
{
  ...
  {
    try {
      PreparedStatement stmt = con.prepareStatement("insert into vendas_pendentes(id_venda,apelido_comprador,qtd,id_produto,email,forma_pagamento) values(?,?,?,?,?,?);");
      ...
      Principal.setTextArea( "Venda: " + ven.getId_venda() + " adicionada como pendente\n");
    }
  ...
  public ArrayList<VendasPendentes> getVendasPendentes(){
    ...
    try {
      ...
      while (rs.next()) {
        VendasPendentes vendasPen = new VendasPendentes();
        ...
        ((Produtos)VariaveisDeControle.mapProd.get(vendasPen.getIdProduto())).getNomeProdutoQtdTotal(vendasPen.getQtd());
        list.add(vendasPen);
      }
      return list;
